define(["setTheStyle", "../lib/three.js/three", "../lib/three.js/orbitControls", "../lib/three.js/firstPersonControls", "../lib/three.js/projector", "../lib/three.js/canvasRenderer", "../lib/three.js/stereoEffect", "../lib/three.js/deviceOrientationControls", "../lib/three.js/stats.min"], function(setTheStyle, THREE, orbitControls, firstPersonControls, Projector, CanvasRenderer, StereoEffect, DeviceOrientationControls, stats) {
    ...
    var iconLogoImages = ['logoP.png', 'logoDungi.png', 'logoTata.png', 'logoSlack.png', 'logoTE.png', 'logoArx.png', 'logoDigi.png', 'logosaAuto.png', 'rsystems.png'];
    ...
    function addEvents()
    {
        ...
        $('#btnh5').on('click', function(e){
            var url = 'https://www.dropbox.com/s/88ikgggqw89i1z5/rloop%20whitepaper%20v1.0.pdf?dl=0';
            window.open(url,'_blank');
            //Clicked on one pager
        })
  ...
  // draw(20);
        ...
        for (q = 0;q<iconLogoImages.length;q++)
        {
            ...
            switch (q) {
                ...
                case 7:
                    ...
                    break;
                case 8:
                    spr.position.set(3, -0.5, 0)
                    spr.scale.set(4.4,4.4,4.4)
                    break;
            }
    ...
    function resizeAllVerexes ( sizeratio )
    {
        var allparticlessSizes = rloop.bufferParticles.geometry.attributes.size.array;
        var allparticles = rloop.particles.geometry.vertices;
        for ( var par = 0; par < allparticlessSizes.length; par++ )
        {   
            if (allparticles[par]) {
                
                if (allparticles[par].dest2) {
                    //console.log('change dest part: ', allparticles[par].dest2);
                    allparticles[par].dest2.size = allparticles[par].dest2.size/sizeratio;
                }
            }
            var part = allparticlessSizes[par];
            var dest = { 
                size: part / sizeratio
            }
    ...
    function onWindowResize(e){
        ...
        if (zoomFromStandard<1) { //} && window.devicePixelRatio<=1) {
            ...
            camFOV = 45;
            if (zoomFromStandard>=1.1) camFOV = 50;
            if (zoomFromStandard>=1.2) camFOV = 53;
            if (zoomFromStandard>=1.3) camFOV = 56;
            if (zoomFromStandard>=1.4) camFOV = 59;
            rloop.camera.fov = camFOV;
            //rloop.camera.aspect = th.width / th.height;
            rloop.camera.updateProjectionMatrix();
        }

        var newDefaultPixedSize = 1 * (zoomFromStandard*1.2);
        if (zoomFromStandard>1 || rloop.mobile) newDefaultPixedSize = 1;
        if (heightZoomFromStandard < 1 && !rloop.mobile && newDefaultPixedSize>=1 )
            newDefaultPixedSize = 1 * heightZoomFromStandard * 1.2;
        if (newDefaultPixedSize>1.1) newDefaultPixedSize = 1.1;

        var pixelDifferenceRatio = defaultPixedSize/newDefaultPixedSize;
        if (pixelDifferenceRatio!=1)
        {
            resizeAllVerexes(pixelDifferenceRatio);
